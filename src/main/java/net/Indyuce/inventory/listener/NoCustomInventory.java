...
public class NoCustomInventory implements Listener {
	...
	public void giveItemsOnJoin(PlayerJoinEvent event) {
		...
		for (CustomSlot slot : MMOInventory.plugin.getSlotManager().getCustomSlots())
			if (UtilityMethods.isAir(player.getInventory().getItem(slot.getIndex())))
	...
	public void placeBackSlotItems(InventoryClickEvent event) {
		...
		if (UtilityMethods.isAir(event.getCursor()) && Utils.isGuiItem(item)) {
			event.setCancelled(true);
			return;
		}
		...
		if (!UtilityMethods.isAir(event.getCursor())) {
			...
			if (!slot.canHost(MMOInventory.plugin.getDataManager().get(player), cursor)) {
				event.setCancelled(true);
				return;
			}
		}
		...
		// Call Bukkit event
		ItemEquipEvent.EquipAction action = UtilityMethods.isAir(event.getCursor()) ? ItemEquipEvent.EquipAction.UNEQUIP : Utils.isGuiItem(cursor) ? ItemEquipEvent.EquipAction.EQUIP : ItemEquipEvent.EquipAction.SWAP_ITEMS;
